          ...
          {SERVICES_DATA.map((service, index) => {
            ...
            return (
              <RevealOnScroll key={service.id}>
                <div
                  className={`p-8 sm:p-12 rounded-3xl bg-[#F8F9FC] border border-[#ECECF1] grid grid-cols-1 lg:grid-cols-12 gap-8 items-center ${
                    isReversed ? "lg:flex-row-reverse" : ""
                  }`}
                >
                  <div className="lg:col-span-7 space-y-6">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider text-[#5B4CFB] bg-[#EFEDFF]">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                      {service.features.slice(0, 4).map((f) => (
                        <div key={f} className="flex items-center gap-2 text-xs font-semibold text-[#0E0E12]">
                    ...
                    <div className="pt-4">
                      <Link
                        ...
                        className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-[#5B4CFB] text-white text-sm font-semibold hover:bg-[#4939E8] transition-all shadow-md"
                      >
                  ...
                  <div className="lg:col-span-5 bg-white rounded-2xl p-6 border border-[#ECECF1] shadow-xs space-y-4">
                    <h4 className="text-xs font-bold uppercase tracking-widest text-[#9494A0]">
                      Service Milestones
                    </h4>
                    <div className="space-y-3">
                      {service.timeline.map((step) => (
                        <div key={step.step} className="flex items-start gap-3">
                          <span className="w-6 h-6 rounded-md bg-[#EFEDFF] text-[#5B4CFB] text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">
                            {step.step}
                          </span>
                          <div>
                            <span className="text-xs font-bold text-[#0E0E12] block">
                              {step.title}
                            </span>
                            <span className="text-[11px] text-[#5B5B66] line-clamp-1 block">
                              {step.description}
                            </span>
                          </div>
                        </div>
